...
export function TimetablePage() {
  ...
  const csvExport = useMutation({ mutationFn: () => { if (!run.data) throw new Error("The timetable is not loaded yet."); return timetableService.downloadCsv(run.data); }, onSuccess: () => setNotice("CSV export started.") });
  ...
  const data = run.data;
  const actionError = validate.error ?? csvExport.error ?? lock.error;
